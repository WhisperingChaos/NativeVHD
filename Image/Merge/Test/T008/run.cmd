...
setlocal
  ...
  set TEST_FILE_MERGE="%MOUNT_VDISK_DRIVE_LETTER%:\hello.txt"
  echo Hello>%TEST_FILE_MERGE%
  if not exist %TEST_FILE_MERGE% exit /b 1
  ...
  if not exist %TEST_FILE_MERGE% exit /b 1
  set MERGE_FILE_CONTENT=Merge Hello
  echo %MERGE_FILE_CONTENT%>%TEST_FILE_MERGE%
  ...
  set MERGE_LAYER_FILE=%TEST_LAYER_VHD_FILE%
  set MERGE_CANONICAL_BASE_FILE=%TEST_BASE_VHD_FILE%
  call "%IMAGE_METHOD_PATH%\Merge.cmd" "%~dp0\config.cmd" 2>&1
  ...
  if not exist %TEST_FILE_MERGE% exit /b 1
  type %TEST_FILE_MERGE% | findstr /R /C:"%MERGE_FILE_CONTENT%"
  if %errorlevel% NEQ 0 exit /b 1
