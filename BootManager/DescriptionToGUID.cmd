...
(
  ...
  echo ::--
  echo ::-- Purpose:
  echo ::--   Given a search string describing the boot loader entry, find an entry that
  echo ::--   matches this search string.  The search string is compared to the values
  echo ::--   of each boot entry's "description" attribute.  A boot entry is selected
  echo ::--   when it encounters an exact match that must be unique within the set
  ...
  echo ::
  echo ::-- Required: Description absent double quotes, of boot entry to return GUID.
  echo set BOOT_ENTRY_NAME="^<BootEntryName^>"
  echo ::
  echo ::-- Optional: The absolute path, enclosed in double quotes, to the BCD store.  Omit this
  echo ::-- variable to specify the path to the default store.
  echo set BOOT_STORE="^<BCDstoreAbsoluteFilePath^>"
  echo ::
...
setlocal
  ...
  call "%BIND_ARGUMENT%\Check" ARGUMENT_CHECK_EMPTY BOOT_ENTRY_NAME_GUID
  if %errorlevel% neq 0 (
  ...
  call :Inform "Find BCD boot entry: Name/GUID: " %BOOT_ENTRY_NAME_GUID%
  
  call bcdedit /v | findstr /R /C:"identifier" | findstr /R /C:%BOOT_ENTRY_NAME_GUID%
