...
:Help:
  ...
  echo ::-->&2
  echo ::-- Purpose:>&2
  echo ::--   Dismount a VHD from the filesystem.>&2
  echo ::--   Note - this works at disk level :: it dismounts all volume mounts.>&2
  echo ::-->&2
  echo ::-- Assumes:>&2
...
setlocal
  ...
  if not exist %MOUNT_REMOVE_VDISK_FILE% (
    call :Abort "MOUNT_REMOVE_VDISK_FILE must exist to be dismounted:'" %MOUNT_REMOVE_VDISK_FILE% "' does not exist or inaccessible due to permissions."
    exit /b 1
  )
