...
:Help:
  ...
  echo ::-->&2
  echo ::--  Purpose:>&2
  echo ::--   Given an existing VHD file, enable its file readonly attribute.>&2
  echo ::--   This action ensures the VHD remains immutable preventing layers>&2
  echo ::--   derived from this one from being inadvertenly corrupted.>&2
  echo ::-->&2
  echo ::-- Assumes:>&2
  ...
  echo ::-- whose read only attribute will be enabled to prevent processes from writing>&2
  echo ::-- to it.  Writes to this file will cause corruption to any child/derived images.>&2
  echo set PROTECT_FILE="<VHDBaseAbsoluteFilePath>">&2
  echo ::>&2
...
setlocal
  ...
  call "%BIND_ARGUMENT%\Check" ARGUMENT_CHECK_EMPTY PROTECT_FILE
  if %errorlevel% neq 0 (
  ...
  call :Inform "Started: VHD: '" %PROTECT_FILE% "' protect"
  if not exist %PROTECT_FILE% (
    call :Abort "PROTECT_FILE must exist:'" %PROTECT_FILE% "' does not exist or inaccessible due to permissions."
    exit /b 1
  )
  attrib +R %PROTECT_FILE% > nul
  if %errorlevel% neq 0 (
    call :Abort "Unable to mark PROTECT_FILE: '" %PROTECT_FILE% "' as read only.  Prevents VHD from being inadvertently deleted/changed at file level."
    exit /b 1
  )
  call :Inform "Ended: VHD: '" %PROTECT_FILE% "' protect: Successful"
